...
const ACCESS_ISSUED_AT_KEY = "community.accessIssuedAt";
const ACCESS_REFRESH_AFTER_MS = 9 * 60 * 1000;
...
async function send(path, options) {
  ...
  if (
    networkOptions.body &&
    !(networkOptions.body instanceof FormData) &&
...
function markAccessTokenIssued() {
  sessionStorage.setItem(ACCESS_ISSUED_AT_KEY, String(Date.now()));
}

function shouldRefreshBeforeRequest(pathname, options) {
  if (options.__skipRefresh || REFRESH_EXCLUDED.has(pathname)) return false;
  const issuedAt = Number(sessionStorage.getItem(ACCESS_ISSUED_AT_KEY));
  return issuedAt > 0 && Date.now() - issuedAt >= ACCESS_REFRESH_AFTER_MS;
}

async function refresh() {
  if (!refreshPromise) {
    refreshPromise = send("/api/users/refresh", {
      method: "POST",
      __skipRefresh: true,
    })
      .then((response) => {
...
export async function apiRequest(path, options = {}) {
  ...
  if (shouldRefreshBeforeRequest(pathname, options)) {
    try {
      await refresh();
    } catch (error) {
      expireSession();
  ...
  try {
    ...
    if (error instanceof TypeError)
      throw new ApiError(
        `백엔드 서버에 연결할 수 없습니다. ${baseUrl()} 실행 여부와 CORS 설정을 확인해주세요.`,
      );
    throw error;
  }
  ...
  if (
    isExpired(response, body) &&
    !options.__retried &&
    !options.__skipRefresh &&
    !REFRESH_EXCLUDED.has(pathname) &&
  ...
  if (pathname === "/api/users/login") markAccessTokenIssued();
  return body?.data ?? body;
}

export function __resetHttpClientForTests() {
  refreshPromise = null;
  sessionExpiryHandled = false;
  sessionStorage.removeItem(ACCESS_ISSUED_AT_KEY);
}
