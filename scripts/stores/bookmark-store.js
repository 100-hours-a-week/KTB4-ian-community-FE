...
export class BookmarkStore extends EventTarget {
  constructor(storage = globalThis.localStorage) {
    ...
    this.handleStorage = (event) => {
      if (event.key === STORAGE_KEY)
        this.dispatchEvent(
          new CustomEvent("change", { detail: { external: true } }),
        );
    };
  ...
  has(postId) {
    return this.all().some((item) => String(item.postId) === String(postId));
  }

  remove(postId) {
    if (!this.has(postId)) return false;
    this.toggle({ postId });
    return true;
  }

  subscribe(listener) {
    const handler = (event) => listener(event.detail, this.all());
  ...
  toggle(post) {
    ...
    if (bookmarked) items.unshift(post);
    else items.splice(index, 1);
    this.storage.setItem(STORAGE_KEY, JSON.stringify(items));
    this.dispatchEvent(
      new CustomEvent("change", {
        detail: { postId: post.postId, bookmarked },
      }),
    );
    return bookmarked;
  }
