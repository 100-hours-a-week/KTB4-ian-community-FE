#!/usr/bin/env python3
"""Generate overlay, diff, and JSON artifacts for comment-edit Modal states."""
...
CANONICAL = VISUAL / "reference" / "comment-edit-modal.png"
COMPARE = VISUAL / "scripts" / "compare_visual.py"
STATES = (
    "comment-edit-modal",
    "comment-edit-modal-validation",
    "comment-edit-modal-enabled",
    "comment-edit-modal-pending",
    "comment-edit-modal-failure",
)
...
def compare(state: str, reference: Path | None = None) -> None:
    ...
    if state != "comment-edit-modal" and not supplied_reference:
...
def main() -> None:
    crop_reference = VISUAL / "reference" / "comment-edit-modal-crop.png"
    Image.open(CANONICAL).crop((720, 367, 1200, 713)).save(crop_reference)
    for state in STATES:
        compare(state)
    compare("comment-edit-modal-crop", crop_reference)
